 ...
 */
async function scaffoldDeclarativeFromV4Channel(context, locator, answers, callerFloor, telemetryProps, flagReader, resolvedPackage) {
    const { source, bytes } = resolveChannelPackageBytes(context, telemetryProps, resolvedPackage);
    const loaded = (0, v4_1.openDeclarativePackage)(bytes, locator);
    if (loaded.isErr()) {
        throw loaded.error;
    }
    ...
    }, (0, v4_1.createRealRuntime)(context.destination, flagReader));
    if (result.isErr()) {
        throw result.error;
    }

...
exports.fileOperationDeps = void 0;
...
const createZip = () => new adm_zip_1.default();
const writeZip = async (zip, cache) => await new Promise((resolve, reject) => {
...
async function zipFolderAsync(sourceDir, cache, notIncluded) {
    ...
    if (exports.fileOperationDeps.existsSync(cache)) {
        try {
            await exports.fileOperationDeps.remove(cache);
        }
        catch (e) {
            if (e instanceof Error && e?.code === "EBUSY") {
                throw new deploy_1.CacheFileInUse(cache, e);
            }
            throw e;
        }
    }
    const zip = exports.fileOperationDeps.createZip();
    ...
    if (cache && tasks) {
        await exports.fileOperationDeps.mkdirs(path_1.default.dirname(cache));
        try {
            await exports.fileOperationDeps.writeZip(zip, cache);
        }
        catch (e) {
            if (e instanceof Error && e?.code === "ERR_OUT_OF_RANGE") {
                throw new deploy_1.ZipFileError(e);
            }
        }
    }
    return exports.fileOperationDeps.createReadStream(cache);
}
